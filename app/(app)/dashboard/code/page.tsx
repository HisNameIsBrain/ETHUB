              ...
              {!loadingDir &&
                entries.map((e) => {
                  ...
                  return (
                    <button
                      key={path}
                      onClick={() => (e.isDir ? loadDir(path) : loadFile(path))}
                      className={[
                    ...
                    >
                      <span className="text-slate-400">
                        {e.isDir ? <Folder className="h-3.5 w-3.5" /> : <File className="h-3.5 w-3.5" />}
                      </span>
            ...
            {currentFile ? (
              ...
              <div className="flex h-full items-center justify-center bg-slate-950/90">
                <p className="max-w-sm text-center text-[11px] text-slate-500">
                  Select a file from the <span className="font-semibold">ETHUB FILES</span> panel to start editing. This editor is powered by{" "}
                  <span className="font-semibold">Monaco</span>, similar to VS Code, but designed to run smoothly even on mobile.
                </p>
              </div>
            )}
          </div>
